...
reg_dfw <- read_csv("../data/02_reg.csv") %>%
  janitor::clean_names() %>%
  mutate(rep = as.factor(rep)) # change the column rep from "dbl" to "fctr"
...
ggplot(reg_dfw, 
       aes(x = sr_ksha, 
           y = yield_mgha)) +
  geom_point() +
  geom_smooth() # Usefull for understand what type of trend i have
...
mod1_int_aug <- augment(mod1_int) %>%
  mutate (.stdresid = resid(mod1_int, # Standardized the residuals
                            type = "pearson",
                            scale = TRUE
                            )) %>%
  left_join(reg_dfw) # Joined my data set with the model
...
ggplot(mod1_int_aug,
  ...
  geom_point(size = 3,
             alpha = 0.7 # Transperancy (goes from 0 to 1, 1 = non transparent)
             ) +
  geom_hline(yintercept = 0, # Give me a line at 0 in the y axis
             color= "red"
             ) +
  geom_smooth() # to see how the values trend
...
ggplot(mod1_int_aug, # See if the residuals are normal distributed
       aes(sample = .stdresid)
       ) +
  stat_qq() + # calculate the stats for the Quantile
  stat_qq_line()
...
ggplot(mod1_int_aug, 
  ...
  geom_point(size = 3, 
             alpha = .7) +
  geom_line(aes(y = .fixed), # it create a line for the intercept
            color = "blue"
            )
```
...
```{r nd}
nd <- data.frame(sr_ksha = seq(40, 120, 1)) # it creates a database that contain the values for our regression line
...
nd <- nd %>%
  bind_cols(mod1_int_boots) # bind nd and mod1_int_boots, so i have 4 colomuns (sr_kshaa, mod1_yield_mgha, mod1_int_lcl, mod1_int_ucl)
...
# Model fitting
mod4_linp <- nlme(yield_mgha ~ SSlinp(sr_ksha, # nlme = non-linear-mixed-effect models
                                      a,b,xs),
                  data = reg_dfw,
...
```
Based on the above, model 4 (linear-plateau) had the lowest AIC and thus should be used to find the optimum level of seeding rate. If the models are more than 2 points different the model with the lower value is better. In this case the LINP has 7 points differences to the QUAD.
